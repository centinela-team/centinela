...
def configure_azure_monitor(service_name: str, connection_string: Optional[str] = None) -> bool:
    """
    Activa Azure Monitor OpenTelemetry si hay connection string.
    Devuelve True si quedó configurado; False si se omite (local sin App Insights).
    """
    conn = (connection_string or os.environ.get("APPLICATIONINSIGHTS_CONNECTION_STRING", "")).strip()
    if not conn:
        logger.info("App Insights omitido (sin APPLICATIONINSIGHTS_CONNECTION_STRING) service=%s", service_name)
        return False
    try:
        ...
        return True
    except Exception as exc:  # noqa: BLE001 — no tumbar el proceso por telemetría
