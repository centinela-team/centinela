...
- **#4 — Resuelto.** Matriz de permisos por rol creada a nivel de infraestructura Azure (no solo de la app): rol custom `Centinela Analista`, mapeo `Auditor`→`Reader`, `Administrador`→`Contributor`. Las 3 pruebas negativas requeridas por el README se verificaron en vivo contra el motor de autorización real de Azure (`Microsoft.Authorization/checkAccess`) y las 3 dieron `denegado`: Servicio (identidad gestionada real de `ca-centinela-api-dev`), Analista y Auditor (cuenta invitada externa creada solo para la prueba y retirada al terminar, ya que crear service principals estaba bloqueado por una restricción de Azure AD del tenant). Detalle completo en `decisions.md`, sección "RBAC de identidad por rol".
