...
| 6 | Push a main → CI/CD | Integración dispara `.github/workflows/ci.yml` (tests + imágenes) y `deploy-azure` (activo desde 2026-07-30): login OIDC sin secretos + `az containerapp update` real sobre las 4 apps. Evidencia real de un rollout automático de punta a punta en `docs/architecture/decisions.md` §CI/CD real |
