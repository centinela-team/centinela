...
3. **Pre-flight de resource providers**: el script verifica los 6 namespaces necesarios antes del `what-if`. Si alguno está `NotRegistered`, intenta registrarlo automáticamente. Si el usuario no tiene permisos de suscripción (caso normal del equipo), el script muere con un mensaje claro que lista exactamente los comandos que debe ejecutar el Owner de la suscripción.
